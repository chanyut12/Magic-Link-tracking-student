...
async function createTask(req, res) {
  ...
  const linkId = uuidv4();
  let expiresHours = parseInt(req.body.expires_in_hours) || DEFAULT_EXPIRY_HOURS;
  if (taskType === 'ATTENDANCE') {
    expiresHours = 24; // Attendance links last for exactly 1 day
  }
...
function submitTaskAttendance(req, res) {
  ...
  try {
    ...
    // After success, check if all students in the class have been checked today.
    const totalStudentsRow = db.prepare('SELECT COUNT(*) as count FROM students WHERE grade = ? AND room = ?').get(link.target_grade, link.target_room);
    const totalStudents = totalStudentsRow ? totalStudentsRow.count : 0;

    const checkedStudentsRow = db.prepare(`
      SELECT COUNT(ar.id) as count 
      FROM attendance_records ar
      JOIN students s ON s.id = ar.student_id
      WHERE s.grade = ? AND s.room = ? AND ar.check_date = ? AND ar.status != 'NOT_CHECKED'
    `).get(link.target_grade, link.target_room, today);
    const checkedStudents = checkedStudentsRow ? checkedStudentsRow.count : 0;

    let messageStr = '';
    if (totalStudents > 0 && checkedStudents >= totalStudents) {
      // All students checked -> Mark as COMPLETED
      db.prepare(`UPDATE task_links SET status = 'COMPLETED' WHERE token_hash = ?`).run(tokenHash);
      messageStr = 'บันทึกสำเร็จและเช็คชื่อนักเรียนครบทุกคนแล้ว ลิงก์ถูกตั้งเป็นสถานะเสร็จสิ้น';
    } else {
      // Not all students checked -> Mark as ACTIVE (in case it was previously COMPLETED)
      db.prepare(`UPDATE task_links SET status = 'ACTIVE' WHERE token_hash = ?`).run(tokenHash);
      messageStr = `บันทึกสำเร็จเรียบร้อย (เช็คแล้ว ${checkedStudents}/${totalStudents} คน) ลิงก์ยังสามารถใช้เช็คชื่อต่อได้จนหมดอายุ`;
    }

    res.json({ success: true, message: messageStr });
  } catch (err) {
